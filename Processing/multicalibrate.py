...
def plot_norm_pos(pupils, num = 1000):
...
#    plot_norm_pos(pupils, 5000)
    ...
    # Compute smoother recv_ts by regressing the pupil
    # timestamps.
    # remove bracket 10 % since there is a dodgy jump.
    ten = int(len(ts) * .1)
    pupil_to_recv = np.polyfit(ts[ten:-ten], rts[ten:-ten], 1)
    new_rts = np.poly1d(pupil_to_recv)(ts)

    pack = lambda x: msgpack.packb(x, use_bin_type=True)
    with open(outpath, 'wb') as out:
        for t, rt, nrt, pos, conf in zip(ts, rts, new_rts, mean, ws):
            row = dict(
                    topic=topic,
                    norm_pos=pos.tolist(),
                    confidence=conf,
                    ...
                    orig_recv_ts=rt,
                    recv_ts=nrt,
                    pupil_to_recv=pupil_to_recv.tolist())
...
def main(dumpdir, outdir, gazefile):
    #import argh
    #argh.dispatch_command(recalibrate_session)

    for fn in glob(dumpdir + "/*.gz"):
        
        print(fn)
        
        #pick participant folder name.

        pp_begin = fn.find("Trout_", len(dumpdir))
        ext_begin= fn.find(".pupil", len(dumpdir))

        pp_fn = fn[pp_begin:ext_begin]

        outfolder = os.path.split(outdir)[1] #outdir already given at the individual level.
        if outfolder == pp_fn:
            outpath = os.path.join(outdir, "000",gazefile)
        else:
            outpath = os.path.join(outdir, pp_fn, "000",gazefile)

        print(outpath)  
        
        recalibrate_session(fn, outpath)


if __name__ == '__main__':

    dumpdir = "C:/git_repos/sample_gaze_Trout/pupildumps"
    outdir = "C:/git_repos/sample_gaze_Trout"
    gazefile = "gaze_recalib_normed.pldata"
    main(dumpdir, outdir, gazefile)