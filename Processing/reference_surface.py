...
class Reference_Surface(object):
    ...
    def map_datum_to_surface(d,m_from_screen):
        ...
        #edited 24/07/19 to deal with retain viz.tick() in recv_ts
        #edited 18/11/19 to catch key errors in recalib_std.
        try:
            recalib_std = d['recalib_std']
        except KeyError:
            recalib_std = [],[]        
        
        #return {'topic':d['topic']+"_on_surface",'norm_pos':(mapped_pos[0],mapped_pos[1]),'confidence':d['confidence'],'on_srf':on_srf,'recv_ts':d['recv_ts'],'timestamp':d['timestamp'],'base_data':d }
        return {'topic':d['topic']+"_on_surface",'norm_pos':(mapped_pos[0],mapped_pos[1]),'confidence':d['confidence'],'on_srf':on_srf,'timestamp':d['timestamp'],'base_data':d, 'recalib_std':recalib_std}

    def map_data_to_surface(self,data,m_from_screen):
        #print ("map to surface data: ", data)
        #data is empty! 
