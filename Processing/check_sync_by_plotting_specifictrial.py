...
def load_pickled_camera(file):
    ...
    if b'rect_map' not in camera:
        ...
        rect_camera_matrix, roi = cv2.getOptimalNewCameraMatrix(camera_matrix, camera_distortion, image_resolution, 0.0, new_imag_size = (1920, 1080))
...
if __name__ == '__main__':
    ...
    #rootdir = "E:/Trout_editedvids_copy/"

    rerun = False
    if rerun:
        rootdir = "F:/Edited_Trout_rerun_Gaze_Videos/"
        trialcode = "2_506_1_1_0"
    else:
        rootdir = "E:/EyeTrike_Backup/Recordings/Trout/ExperimentProper/"
        trialcode = "1_10_0_0_0"
    
    print(trialcode)
    block, ppid, sectiontype, condition, count =  trialcode.split("_") 

    rootdir = rootdir + 'Trout_' + ppid + '_' + block
    ...
    #steergazefile = "C:/git_repos/Trout18_Analysis/Post-processing/GazeAndSteering_newlatency_tidied_3D_remapped_addsample.csv"

    steergazefile = "C:/git_repos/Trout18_Analysis/Data/trout18_gazeandsteering_2019-11-21 - Copy.csv"
...
#    steergaze = steergaze.query("ID == 501 and Block==3")
    ...
    steergaze = steergaze.query(query_string)
    #steergaze = steergaze.loc(steergaze['trialcode'] == trialcode, :)
    ...
    for dirs in os.walk(rootdir):
        ...
        if os.path.exists(marker_path):   
            ...
            filepath = path_3D + gazedata_filename
            print(filepath)
            gazedata = pd.read_csv(filepath)

            ##find closest time in the world frame.
            start_frames_trial = 150
            pprint(steergaze)
            starttime_viz = steergaze.loc[steergaze['f'] > start_frames_trial, 'currtime'].values[0] + latency #get vizard time, corrected by latency.
            
            recv_ts = gazedata['viz_timestamp'].values
            min_idx = recv_ts.searchsorted(starttime_viz) #find closest time in file.
            start_idx = gazedata.loc[min_idx, 'world_frame_idx']
            print(start_idx)
            print("here2")
            ...
            for idx, marker_frame_ in enumerate(marker_cache):
               ...
               # print(idx)
                ...
                closest_recv_timestamp = gazedata.loc[gazedata['world_frame_idx']==idx, 'viz_timestamp']                                
                ...
                recv_ts_array = gazedata['viz_timestamp'].values
               ...
               # gaze_on_screen = row['hangle_new'], row['vangle_new']
                ...
                plt.plot(inside_edge_pixels[:,0], inside_edge_pixels[:, 1], 'k.', markersize = .5)
                plt.plot(outside_edge_pixels[:,0], outside_edge_pixels[:, 1], 'k.', markersize = .5)
                ...
                for target in target_circles:
                   ...
                   # target_pixels = target_pixels[target_pixels[:,1]<cy,:]
                    ...
                    plt.plot(target_pixels[:,0], target_pixels[:,1], mycolour, markersize = .2)
                ...
                screen_coords *= screen_res


                if row['confidence'] > .4: #generous
                    
                    plt.plot(screen_coords[0], screen_coords[1], 'mo', markersize = 5)
                else:
                    pass    
                ...
                surface_outline_vid *= video_res  #in scene camera pixels
                ...
                plt.plot(surface_outline_viz[:,0], surface_outline_viz[:,1], 'g-') #flip.
                ...
                plt.ylim(0, screen_res[1])
                plt.xlim(0, screen_res[0])
                plt.imshow(warp)
                #plt.imshow(cv2.flip(screenshot,0))
                plt.pause(.25)                    
                plt.cla()
            plt.show()   
